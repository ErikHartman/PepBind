...
# This script cleans the downloaded PDBs retrieved with curate_benchmark_data.py.
# It removes any PDBs that Rosetta throws an error for when loading,
# and removes any models after the first one in NMR models.
...
# Some of the PDBs are NMR models, which have multiple models in the same file.
# We only want to keep the first model. Filter through cleaned PDBs and keep only the first model.
