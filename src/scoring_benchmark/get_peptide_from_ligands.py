...
def process_ligands(df):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        future_to_ligand = {
            executor.submit(is_peptide_like, ligand_id): ligand_id
            for ligand_id in unique_ligand_names
        }
        for index, future in enumerate(
            concurrent.futures.as_completed(future_to_ligand)
        ):
...
def main():
    ...
    output_file_non_peptide = os.path.join(
        data_dir, "databases/benchmark_data/peptide_like_ligands.csv"
    )
    ...
    print(
        f"Merged peptide-like ligands have been written to {os.path.join(data_dir, 'output_data/merged_peptide_like_ligands.csv')}"
    )
